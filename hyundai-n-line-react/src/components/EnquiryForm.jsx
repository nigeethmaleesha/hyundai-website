...
export default function EnquiryForm() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: error?.message || 'Unable to send your enquiry right now. Please try again shortly.' })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <form className="enquiry-form" onSubmit={submit}>
      ...
      <div className="form-grid two names-row">
        <label>
          FIRST NAME
      ...
      <label>
        EMAIL ADDRESS
        <input required type="email" name="email" value={form.email} onChange={update} placeholder="john.doe@example.com" />
      </label>

      <label>
        MOBILE NUMBER
        <input required name="mobile" value={form.mobile} onChange={update} placeholder="+94 XXX XXXX" />
      </label>
